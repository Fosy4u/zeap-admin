...
const UserDeleteRestore = ({ close, mode="disable", user}:{
...
<div id="popup-modal" tabIndex={-1} className="overflow-y-auto  fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div className="relative  min-h-screen flex flex-col justify-center items-center bg-slate-100  dark:bg-slate-700 overflow-hidden">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            {(diableUserStatus?.isLoading || enableUserStatus?.isLoading) && <Loading />}
            <button 
            onClick={close}
            ...
            {error && <Banner variant="error" message={error} className="mb-4" />}
                ...
                <button
                onClick={close}
                 data-modal-hide="popup-modal" type="button" className="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200  hover:text-darkGold focus:z-10 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600">No, cancel</button>
            </div>
        </div>
    </div>
</div>
