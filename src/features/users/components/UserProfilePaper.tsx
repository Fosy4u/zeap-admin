      ...
      <>
          ...
          <div className="p-6 ">
            <header className="mb-4">
              ...
              <div className="flex overflow-scroll mt-3 items-center justify-center">
                ...
                <span
                  className={`inline-flex items-center justify-center gap-1 rounded-full  px-1.5 text-sm text-white mr-2 ${user?.disabled ? 'bg-danger' : 'bg-success'}`}
                >
                  {user?.disabled ? 'Disabled' : 'Active'}
                </span>
                {user?.isVendor && (
                  <span className="inline-flex items-center justify-center gap-1 rounded-full bg-slate-600 px-1.5 text-sm text-white mr-2">
                    Vendor
                  </span>
                )}
                {getLevel() && (
        ...
        {open && (
          <SimpleModal
            ...
            onclick={() => {}}
          >
            <SignUp close={() => setOpen(false)} user={user} mode="edit" />
          </SimpleModal>
        )}
      </>
    </div>
