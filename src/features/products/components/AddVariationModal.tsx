...
import { Alert, Button, Label, Modal, Dropdown, TextInput } from "flowbite-react";
...
const AddVariationModal = ({
    ...
    const [addVariation, addVariationStatus] = zeapApiSlice.useAddProductVariationMutation();
    const [editVariation, editVariationStatus] = zeapApiSlice.useEditProductVariationMutation();
    ...
    const handleAddVariation = () => {
        setError(null)
        if(!validate()) return
        const payload = {productId: product?.productId, variation: {colorValue:color, size, price, quantity,
    ...
    return (
        ...
        size="7xl"
          theme={ModalTheme}
         show={open} onClose={() => close()}>
          <Modal.Header>
                {currVariation ? `Edit Variation - ${currVariation?.sku}` : "Add Variation"}
          </Modal.Header>
          <Modal.Body>
              {isLoading && <Loading />}
          {error && (
        ...
        <div className="flex flex-col md:flex-row md:justify-between gap-4">
          <div className="mb-2 flex flex-col gap-1">
            <Label  value="Color" />
            <Dropdown label={color || "Select Color" } size="xs"
            ...
            <div className="mb-2 flex flex-col gap-1">
                <Label  value="Size" />
                <Dropdown label={size || "Select Size"} size="xs"
                color={size ? "success" : "primary"}
                inline ={size ? false : true}
                >
                    {sizes?.map((size, index) => (
                        <Dropdown.Item key={index} onClick={() => setSize(size)}>{size}
