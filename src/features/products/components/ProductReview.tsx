...
const ProductReview = ({ product }: { product: ProductInterface }) => {
                  ...
                  >
                    ...
                    <div className="flex justify-between">
                      <div className="flex items-center gap-4">
                        <Avatar
                          img={review?.user?.imageUrl?.link || NoPic}
                    ...
                    <div className="flex gap-8">
                      <div
                        className={`flex items-center gap-2 ${checkIfUserLiked(review.likes) ? 'pointer-events-none' : ''}`}
                        onClick={() => handleLike(review._id)}
                      >
                        <HiThumbUp
                          className={`${checkIfUserLiked(review.likes) ? 'text-success' : 'text-green-300'}`}
                        />
                      ...
                      <div
                        className={`flex items-center gap-2 ${checkIfUserDisliked(review.dislikes) ? 'pointer-events-none' : ''}`}
                        onClick={() => handleDislike(review._id)}
                      >
                        <HiThumbDown
                          className={`${checkIfUserDisliked(review.dislikes) ? 'text-danger' : 'text-red-300'}`}
                        />
                        <p className="text-xs text-slate-400">
