...
const AddImageToColor = ({
    ...
    const executeScroll = () => myRef.current ? myRef.current.scrollIntoView({ behavior: "smooth" }) : null;
    const [selectedFiles, setSelectedFiles] = useState<FileList | null>(null);

    const [previews, setPreviews] = useState<PreviewInterface[]>([]);
    const [errorMsg, setErrorMsg] = useState<string>("");
    const [updateProductColorAndImages, updateProductColorAndImagesStatus] = zeapApiSlice.useUpdateProductColorAndImagesMutation()
    const[addImagesToProductColor, addImagesToProductColorStatus] = zeapApiSlice.useAddImagesToProductColorMutation();
    const isLoading = updateProductColorAndImagesStatus.isLoading || addImagesToProductColorStatus.isLoading;
        ...
        if (selectedFiles) {
            for (let i = 0; i < selectedFiles.length; i++) {
                ...
                if (fileSizeKiloBytes > MAX_FILE_SIZE) {
                    executeScroll()
                    setErrorMsg("Selected file size is greater than 1MB. Please select a smaller file");
                    setTimeout(() => {
    ...
    const handleAddImages = async () => {
        ...
        if(currColor){
            addImagesToProductColor({payload}).unwrap().then(() => {
                close();
            }
            ).catch((err) => {
                executeScroll()
        ...
        updateProductColorAndImages({payload}).unwrap().then(() => {
            close();
        }
        ).catch((err) => {
            executeScroll()
    ...
    return (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5  bg-bodydark my-2">
                {currColor?.images?.map((image, index) => (
                    <div key={index} className="flex wrap gap-2 ">
                        ...
                        <img  key={index} src={image?.link} alt={color} className="w-full h-50 object-contain" />
                        </div>
                    </div>
                ))}
        ...
        {previews.map((preview, index) => (
            <div key={index} className="relative   my-2 p-2">
                ...
                ref ={index === 0 ? myRef : null}
                 src={preview?.preview} alt="preview" className="w-full h-50 object-contain" />
                <button
                    onClick={() => {
                        if (selectedFiles) {
      ...
      >
        <div className="flex flex-col items-center justify-center pb-6 pt-5">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">Must not exceed 5 images</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG, WEBP or AVIF (MAX. 1MB)</p>
        </div>
        ...
        onChange={(e) => {
            ...
            if(selectedFiles){
                const dataTransfer = new DataTransfer();
                for (let i = 0; e.target.files && i < e.target.files.length; i++) {
                ...
                if(dataTransfer.files?.length > 5){
                    executeScroll()
                    setErrorMsg("You can only upload a maximum of 5 images for each color");
                    setTimeout(() => {
