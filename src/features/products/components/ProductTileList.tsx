      ...
      <div className="md:hidden grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4  gap-5 w-full items-center justify-center">
        {products?.map((product: ProductInterface) => (
          <NavLink key={product?.productId} to={`/product/${product?._id}`}>
            <div className="flex flex-col light:bg-grey8 gap-2 p-2 my-2 cursor-pointer rounded-lg shadow-md duration-300 hover:scale-105 hover:shadow-lg  ">
              <img
              ...
              <div className="flex  flex-col">
                <span className="flex justify-between">
                  {product?.variations[0]?.price ? (
                    <p className="mr-2 text-md font-semibold text-gray-900 dark:text-white">
                      {currency?.symbol}
                      {product?.variations[0]?.discount?.discountPrice ||
                        product?.variations[0]?.price}
                    </p>
                  ) : (
                ...
                {product?.variations[0]?.discount?.percentage && (
                  <p className="text-xs text-green-500">
                    {product?.variations[0]?.discount?.percentage}% off
                  </p>
                )}
              </div>
            </div>
